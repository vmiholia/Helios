...
import { Trash2, ChevronDown, ChevronUp, Info, AlertCircle } from 'lucide-react';

const EntryCard = ({ entry }: { entry: any }) => {
    const { deleteEntry } = useHealthStore();
        ...
        >
            <div className="flex items-center justify-between p-3 cursor-pointer" onClick={() => setExpanded(!expanded)}>
                <div className="flex flex-col gap-1">
                    ...
                    <span className="text-[10px] text-neutral-500 font-mono uppercase tracking-wider group-hover:text-cyan-600 transition-colors">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-end gap-1">
                        <span className="font-mono text-xs text-neutral-400">{entry.macros.calories} kcal</span>
                        <span className="font-mono text-[10px] text-neutral-600">{entry.macros.protein}g P</span>
                    </div>
                    {/* Delete Action */}
                    <button
